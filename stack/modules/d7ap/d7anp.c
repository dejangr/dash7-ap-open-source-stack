...
void d7anp_start_foreground_scan()
{
    if(fg_scan_timeout_ticks > 0)
    {
        ...
        {
            // Tl timer is normally ongoing, update only the tl timeout value
...
void d7anp_stop_foreground_scan()
{
    if (d7anp_state == D7ANP_STATE_FOREGROUND_SCAN)
    {
        // stop the foreground scan
        cancel_foreground_scan_task();
        // don't start the automation scan, radio is set to idle
        dll_stop_foreground_scan(false);
        switch_state(D7ANP_STATE_IDLE);
    }
}
